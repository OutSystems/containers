...
node {
    ...
    try {
        psScript = """
        ${baseJenkinsPath}/pipelines/ContainerRun.ps1 -MachineFullyQualifiedName "site.domain.amazing.com" `
        ...
        def errorJson = "{\"Error\":{\"Message\": \"Jenkins - 'Container Run' failed.\"}}"

        def filePath = resultPath + "/" + applicationKey + "_" + operationId

        new File(filePath + ".deploydone").write(errorJson)
