...
node {
    ...
    try {
        psScript = """
        ${baseJenkinsPath}/pipelines/ContainerBuild.ps1 -SiteName "Default Web Site" `
        ...
        def errorJson = "{\"Error\":{\"Message\": \"Jenkins - 'Container Build' failed.\"}}"

        def filePath = resultPath + "/" + applicationKey + "_" + operationId

        new File(filePath + ".preparedone").write(errorJson)
