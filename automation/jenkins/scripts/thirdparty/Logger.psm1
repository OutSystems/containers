...
$logFileName = "containerbundlewatcher-$(Get-Content env:computername).log"
...
Function WriteLogLine ($line) {
    Add-Content $global:logFile -Value $Line -Encoding UTF8
    Write-Host $Line
}
...
Function WriteLog {
    ...
    if ($logLevelPos -lt 0) {
        WriteLogLine "$Stamp ERROR Wrong logLevel configuration [$logLevel]"
    }
...
$Null = @(
    ResetLog -fileName $logFile -filesize $logSize -logcount $logCount
)